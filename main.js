...
async function generateMusic() {
    ...
    try {
        ...
        if (!res.ok) {
            error.innerText = `❌ ${data.error || "Server error occurred."}`;
            return;
        }

        if (!data.notes || data.notes.length === 0) {
            error.innerText = `❌ No music notes generated.`;
            return;
        }
        ...
        // === Use backend URL for MIDI download link ===
        downloadLink.href = BACKEND_URL + data.midi_url;
        ...
        loading.classList.add("hidden");
        error.innerText = "❌ Failed to connect to backend or invalid response.";
